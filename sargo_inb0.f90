...
integer, allocatable:: xrf(:), ped2(:,:)
real:: tot_inb, t1, t2, t11, t22
real, allocatable:: dd(:), inb(:), A22(:,:)
character*40:: fped
...
n=count_lines(unped)
allocate(ped(n,3),psigen(n),dd(n),inb(n))
...
do k=3, maxgen
        ...
        call cpu_time(t11)
        allocate(ispar(bgen(k-1)),xrf(bgen(k-1)))
        ...
        allocate(ped2(bgen(k-1),3),A22(npar,npar))
        ped2=ped(1:bgen(k-1),:)
        call colleau3(ped2,ispar,dd(1:bgen(k-1)),bgen(k-1),A22,npar)
...
!subroutine read_ped(unped,ped,n)
!implicit none
!integer:: n, i, unped, ped(n,3)
!do i=1,n
!        read(unped,*) ped(i,:)
!enddo
!end subroutine read_ped
